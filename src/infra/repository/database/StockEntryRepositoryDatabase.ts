...
export default class StockEntryRepositoryDatabase implements StockEntryRepository {
  ...
  async getByIdItem(idItem: number): Promise<StockEntry[]> {
    ...
    for (const stockEntryData of stockEntriesData) {
      stockEntries.push(
        new StockEntry(stockEntryData.id_item, stockEntryData.operation, stockEntryData.quantity, new Date())
      );
    }
    return stockEntries;
  }
  ...
  async save(stockEntry: StockEntry): Promise<void> {
    const { idItem, operation, quantity } = stockEntry;
    await this.database.none("insert into ccca.stock_entry (id_item, operation, quantity)  values ($1, $2, $3)", [
      idItem,
      operation,
      quantity,
    ]);
